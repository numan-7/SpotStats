...
const { fetchPlaylists, fetchArtists, userInformation } = require('../utils/spotifyUtils');
...
router.get('/user-artists', async (req, res) => {
  ...
  try {
    const data = await fetchArtists(req.user.accessToken);
    res.json(data);
  } catch (error) {
    console.error('Error fetching Spotify stats:', error);
    res.status(500).json({ message: 'Internal server error' });
...
router.get('/logout', (req, res) => {
  ...
  delete req.user.tokenReceivedAt;
  delete req.user.expires_in;
  req.user = null;
